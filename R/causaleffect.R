...
latent.projection <- function(G, l) {
  ...
  for (i in 1:length(l)) {
    ...
    if (inc.len > 0 & out.len > 0) {
      ...
      G           <- G + igraph::edges(v[c(obs.new)]) # replace path v_1 -> L -> v_2 with v_1 -> v_2
    }
    if (out.len > 1) {
      unobs.new   <- utils::combn(out.ind, 2)
    ...
    e.dat         <- as.data.frame(get.edges(G, igraph::E(G)))
    e.dat[ ,3]    <- igraph::edge.attributes(G)
    G             <- igraph::subgraph.edges(G, which(!duplicated(e.dat)))
  }
...
parse.graphml <- function (file, format = c("standard", "internal"), nodes = c(),use.names = TRUE)
{
  ...
  res <- switch(format, standard = parse.graphml.standard(file,
                                                          nodes, use.names), internal = parse.graphml.internal(file,
                                                                                                               nodes, use.names), stop(paste("Unknown file format:",
                                                                                                                                             format)))
  return(res)
}
...
parse.graphml.standard <- function (file, nodes, use.names)
{
  ...
  graph <- top[["graph"]]
  if (!use.names) {
    if (XML::xmlSize(which(XML::xmlSApply(graph, XML::xmlName) == "node")) !=
        length(nodes))
      stop("Incorrect number of node names")
  }
  ...
  for (i in 1:length(edges)) {
    ...
    for (j in 1:length(datas)) {
      ...
      if (XML::xmlAttrs(current.data) == "d10") {
        ...
        two.arrows <- !identical(arw.attr[["source"]],
                                 "none") & !identical(arw.attr[["target"]],
                                                      "none")
        no.arrows <- identical(arw.attr[["source"]],
                               "none") & identical(arw.attr[["target"]], "none")
        if (two.arrows | no.arrows) {
          remove.id <- remove.id + 1
          removals[[remove.id]] <- current.edge
          e1 <- XML::newXMLNode("edge", parent = doc, attrs = c(id = "e",
                                                           source = src, target = trgt), XML::newXMLNode("data",
                                                                                                    attrs = c(key = "d9"), cdata = TRUE, "U"))
          e2 <- XML::newXMLNode("edge", parent = doc, attrs = c(id = "e",
                                                           source = trgt, target = src), XML::newXMLNode("data",
                                                                                                    attrs = c(key = "d9"), cdata = TRUE, "U"))
  ...
  if (use.names) {
    node.data <- XML::getNodeSet(doc, "//ns:data[contains(@key,'d6')]/*",
                            ns)
  ...
  keep <- XML::getNodeSet(doc, "//ns:edge | //ns:node | //ns:graph | //ns:key[@attr.name = 'description'] | //ns:data[contains(@key,'d9')]",
                     ns)
  remove <- XML::getNodeSet(doc, "//ns:key[@for='port'] | //ns:key[@for='graphml'] | //ns:data[@key='d4'] | //ns:data[@key='d6'] \n | //ns:data[@key='d7'] | //ns:data[@key='d8'] | //ns:data[@key='d10']",
                       ns)
